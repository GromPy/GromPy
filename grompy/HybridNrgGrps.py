...
from ctypes import c_char_p,\
                   ...
                   c_ulong,\
                   c_char
from grompy import libmdrun,rvec,c_real,ivec,stderr,libc,stdout
...
def MCInsert(currmc = gt.mdrunner_comm,
             ...
             SigmaVel = float,
             bGenerate = bool,
             vecxin = [float,float,float],
             vecvin = [float,float,float]):
    vecx = [0.0,0.0,0.0]
    vecv = [0.0,0.0,0.0]
    if(bGenerate):
        vecx[0] = random.random()*trialmc.box[gt.XX][gt.XX]
        vecx[1] = random.random()*trialmc.box[gt.YY][gt.YY]
        vecx[2] = random.random()*trialmc.box[gt.ZZ][gt.ZZ]
        vecv[0] = random.gauss(0.0,SigmaVel)
        vecv[1] = random.gauss(0.0,SigmaVel)
        vecv[2] = random.gauss(0.0,SigmaVel)
    else:
        vecx = vecxin
        vecv = vecvin
    xadd = rvec(c_real(vecx[0]),c_real(vecx[1]),c_real(vecx[2]))
    vadd = rvec(c_real(vecv[0]),c_real(vecv[1]),c_real(vecv[2]))
...
def MCInsertN0(trialmc = gt.mdrunner_comm,
    ...
    trialmc.state.contents.x[Ntrial-1][gt.XX] = xadd[gt.XX];
    trialmc.state.contents.x[Ntrial-1][gt.YY] = xadd[gt.YY];
    trialmc.state.contents.x[Ntrial-1][gt.ZZ] = xadd[gt.ZZ];
    trialmc.state.contents.v[Ntrial-1][gt.XX] = vadd[gt.XX];
    trialmc.state.contents.v[Ntrial-1][gt.YY] = vadd[gt.YY];
    trialmc.state.contents.v[Ntrial-1][gt.ZZ] = vadd[gt.ZZ];
...
#   select particle at random
    if(bGenerate):
        iRemove = c_int(random.randint(0,Ncurr-1))
    else:
...
def RenameOutputFiles(pwd):
    os.rename(pwd+'/ener.edr', pwd+'/ener.edr.bak')
    os.rename(pwd+'/traj.trr',pwd+'/traj.trr.bak')
    os.rename(pwd+'/traj.xtc',pwd+'/traj.xtc.bak')
    os.rename(pwd+'/confout.gro',pwd+'/confout.gro.bak')
...
def hybrid_nrggrps(Mu,NStart,NMax,NMin,TprDir):
    MuRed = Mu*iEps
...
#####################
    ...
    Mass                        = GetMassOfMolType(CurrMc,GCMCMolType)
    SigmaVel                    = math.sqrt(phys.BOLTZ*Temperature/Mass)
...
#    should check what the behavior is for the N=1 and N=0 cases!
#    then NO nrggrps... Etot simply is 0.0

#   as in txtdump.c
#    nat_max = 0
#    for g in range(gt.egcNR.value):
#        nat_max = max(nat_max,DictMdrCmm[CurrNMol].mtop.contents.groups.ngrpnr[g])
    iTrialMol = 0
    MolBlock  = 0 # by definition, because the trial removal group is the first molecule in the topology
    CheckIfTrialMolIsFirstInTopology(DictMdrCmm[CurrNMol],iTrialMol,MolBlock)
    iMolRemove = SelectMoleculeForTrialRemoval(DictMdrCmm[CurrNMol],CurrNMol,MolBlock,iTrialMol)
    ...
    UPotRemove  = GetUPotOfTrialRemoval(DictMdrCmm[CurrNMol])
...
#                       always accept:
                        libmdrun.copy_saved_coords_and_velocities(byref(DictMdrCmm[CurrNMol]))
                        UPot         = GetUPotFromMdebin(DictMdrCmm[CurrNMol],iPot)
                        if(CurrNMol>1):
                            UPotRemove   = GetUPotOfTrialRemoval(DictMdrCmm[CurrNMol])
                        else:
                            UPotRemove   = 0.0
                        ...
                        if(CurrNMol>NMin):
                            ...
                            if(CurrNMol>1):
                                ...
                                if(not DictMdrCmm.has_key(TrialNMol)):
                                    ...
                                    del TrialMc
                                    if(TrialNMol>1):
                                        CheckIfTrialMolIsFirstInTopology(DictMdrCmm[TrialNMol],iTrialMol,MolBlock) # otherwise the energy group doesn.t exist
#                                    iRemove = MCRemove(DictMdrCmm[CurrNMol],DictMdrCmm[TrialNMol],CurrNMol,TrialNMol,False,c_int(0))
...
#                                    RemoveBaks(PWD)
                                    ...
                                    DeltaUPot = 0.0
##                                    iRemove = MCRemove(DictMdrCmm[CurrNMol],DictMdrCmm[TrialNMol],CurrNMol,TrialNMol,True,c_int(-1))
#                                    iRemove = MCRemove(DictMdrCmm[CurrNMol],DictMdrCmm[TrialNMol],CurrNMol,TrialNMol,False,c_int(0))
#                                    DictMdrCmm[TrialNMol].nsteps_done = c_int(0)
#                                    DictMdrCmm[TrialNMol].inputrec.contents.init_step = c_int(0)
#                                    start_t = gt.time_t(mdrunner_integrate(fplog,cr,NFILE,fnm,bVerbose,bCompact,
...
#                                    print math.exp(-Beta*(Mu-UPotRemove))*float(CurrNMol)*InvVol*BroglieLambdaCube,\
#                                          math.exp(-Beta*(Mu+(TrialUPot-CurrUPot)))*float(CurrNMol)*InvVol*BroglieLambdaCube # klopt nog niet!!
#                                    print (-UPotRemove-(TrialUPot-CurrUPot))/TrialUPot
#                                    if previous_move_was_remove => energy evaluation!
#                                    sys.exit(0)
#                                    UPotRemove = GetUPotOfTrialRemoval(DictMdrCmm[TrialNMol])
#                                    libmdrun.clear_mdebin(DictMdrCmm[TrialNMol].mdebin)
#                                    iMolRemove = SelectMoleculeForTrialRemoval(DictMdrCmm[TrialNMol],CurrNMol,MolBlock,iTrialMol)
...
#                               Accepted
                                CurrNMol = TrialNMol
                                if(CurrNMol>0):
                                    iRemove    = MCRemove(DictMdrCmm[CurrNMol+1],DictMdrCmm[CurrNMol],CurrNMol+1,CurrNMol,False,c_int(0))
                                    iMolRemove = SelectMoleculeForTrialRemoval(DictMdrCmm[CurrNMol],CurrNMol,MolBlock,iTrialMol)
                                    DictMdrCmm[CurrNMol].nsteps_done = c_int(0)
                                    DictMdrCmm[CurrNMol].inputrec.contents.init_step = c_int(0)
                                    DictMdrCmm[CurrNMol].inputrec.contents.nsteps = c_int(NSingleP)
                                    start_t = gt.time_t(mdrunner_integrate(fplog,cr,NFILE,fnm,bVerbose,bCompact,
                                                                           dlb_scale,
                                                                           nstepout,ed,repl_ex_nst,repl_ex_seed,
                                                                           cpt_period,max_hours,Flags,
                                    ...
                                    libmdrun.copy_saved_coords_and_velocities(byref(DictMdrCmm[CurrNMol]))
                                    RemoveBaks(PWD)
                                    TrialUPot  = GetUPotFromMdebin(DictMdrCmm[CurrNMol],iPot)
                                    if(CurrNMol==1):
                                        UPotRemove = 0.0
                                        TrialUPot  = 0.0
                                    else:
...
#                       Insert
                        ...
                        if(not DictMdrCmm.has_key(TrialNMol)):
                            ...
                            del TrialMc
                            if(TrialNMol>1):
                                CheckIfTrialMolIsFirstInTopology(DictMdrCmm[TrialNMol],iTrialMol,MolBlock)
                        if(CurrNMol==0):
                            bAccept,\
                            vecx,\
                            vecv = MCInsertN0(DictMdrCmm[TrialNMol],
                                              TrialNMol,
                                              SigmaVel,
                                              True)
                        else:
                            bAccept,\
                            vecx,\
                            vecv = MCInsert(DictMdrCmm[CurrNMol],
                                            ...
                                            True)
                        if(bAccept):
                            iMolRemove = SelectMoleculeForTrialRemoval(DictMdrCmm[TrialNMol],TrialNMol,MolBlock,iTrialMol)
                            DictMdrCmm[TrialNMol].nsteps_done = c_int(0)
                            DictMdrCmm[TrialNMol].inputrec.contents.init_step = c_int(0)
                            DictMdrCmm[TrialNMol].inputrec.contents.nsteps = c_int(NSingleP)
                            start_t = gt.time_t(mdrunner_integrate(fplog,cr,NFILE,fnm,bVerbose,bCompact,
                                                                   dlb_scale,
                                                                   nstepout,ed,repl_ex_nst,repl_ex_seed,
                                                                   cpt_period,max_hours,Flags,
                            ...
                            TrialUPot  = GetUPotFromMdebin(DictMdrCmm[TrialNMol],iPot)
#                            DeltaUPot  = TrialUPot - CurrUPot
                            DeltaUPot  = TrialUPot - RunningUPot
#                            DU         = iEps*(DeltaUPot)
                            Exponent   = Beta*(Mu-DeltaUPot)
#                            Exponent = BetaRed*(MuRed-DU)
...
#            fw.write(str(c)+" "+str(CurrUPot)+" "+str(RunningUPot)+" "+str(Rho)+" "+str(CurrNMol)+" "+str(CountMD)+" "+str(CountRem)+" "+str(CountRemAcc)+" "+str(CountIns)+" "+str(CountInsAcc)+"\n")
        ...
        if(CurrNMol>0):
            ...
            start_t_init = gt.time_t(mdrunner_integrate(fplog,cr,NFILE,fnm,bVerbose,bCompact,
                                                        ...
                                                        byref(DictMdrCmm[CurrNMol])))
            libmdrun.copy_saved_coords_and_velocities(byref(DictMdrCmm[CurrNMol]))
            TotalUPot = GetUPotFromMdebin(DictMdrCmm[CurrNMol],iPot)
            libmdrun.clear_mdebin(DictMdrCmm[CurrNMol].mdebin)
        else:
            TotalUPot = 0.0
        fw.write("# End of sampling:\n")
        fw.write("# ================\n")
        fw.write("# | Single Point Energy - Running Energy | = "+str(abs(TotalUPot-RunningUPot))+" [kJ/mol]\n")
        fw.write("# | Single Point Energy - Running Energy |\n")
        fw.write("# |--------------------------------------| = "+str(abs((TotalUPot-RunningUPot)/max(0.5,RunningUPot)))+" [-]\n")
