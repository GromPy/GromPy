...
############################
    ...
    if(commrec.MASTER(cr)):
        libmdrun.CopyRight.argtypes = [gt.FILE_p,c_char_p]
...
#####################
    ...
    ener                        = ener()
    libmdrun.calc_dispcorr_non_static(fplog,CurrMc.inputrec,CurrMc.fr,c_int(0),
                                      c_int(CurrMc.mtop.contents.natoms),CurrMc.box,
                                      c_real(CurrMc.state.contents.Lambda),
                                      pres,virial,ener)
    del ener,pres,virial  # only interested in dispersion ENERGY
    ...
    for i in range(len(AtomMass)):
        ...
        TotMass += AtomMass[i]
    SigmaVel.append(math.sqrt(phys.BOLTZ*Temperature/TotMass))
    ...
    libmdrun.maxwell_speed(c_real(Temperature),
                           c_int(DictMdrCmm[CurrNMol].inputrec.contents.andersen_seed),
                           DictMdrCmm[CurrNMol].mtop,
                           DictMdrCmm[CurrNMol].state.contents.v)
...
#                                CurrUPot     = RunningUPot
                                ...
                                if(TrialNMol>0):
                                    libmdrun.clear_xsave(byref(DictMdrCmm[TrialNMol]))
                                    libmdrun.clear_vsave(byref(DictMdrCmm[TrialNMol]))
                        CountRem += 1
                    else:
#                       Insert
                        bInsert   = True
...
##############
    ...
    if(commrec.MULTIMASTER(cr)):
        libmdrun.thanx.argtypes = [gt.FILE_p]
